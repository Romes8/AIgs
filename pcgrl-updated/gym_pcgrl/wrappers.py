...
class ToImage(gym.Wrapper):
    def __init__(self, game, names, **kwargs):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=max_value, shape=(self.shape[0], self.shape[1], depth), dtype=np.float32)
    ...
    def transform(self, obs):
        final = np.empty((self.shape[0], self.shape[1], 0), dtype=np.float32)
        for n in self.names:
            obs_component = obs[n].reshape(self.shape[0], self.shape[1], -1)
            final = np.concatenate((final, obs_component), axis=2)
        return final
...
class OneHotEncoding(gym.Wrapper):
    def __init__(self, game, name, **kwargs):
        ...
        assert name in self.env.observation_space.spaces.keys(), f'This wrapper only works for representations that have a {name} key'
        self.name = name
        ...
        self.dim = int(self.observation_space[self.name].high.max() - self.observation_space[self.name].low.min() + 1)
        new_shape = list(shape) + [self.dim]
        self.observation_space.spaces[self.name] = gym.spaces.Box(low=0, high=1, shape=tuple(new_shape), dtype=np.uint8)
...
class Cropped(gym.Wrapper):
    def __init__(self, game, crop_size, pad_value, name, **kwargs):
        ...
        assert 'pos' in self.env.observation_space.spaces.keys(), 'This wrapper only works for representations that have a position'
        assert name in self.env.observation_space.spaces.keys(), f'This wrapper only works if you have a {name} key'
    ...
    def transform(self, obs):
        map_data = obs[self.name]
        x, y = obs['pos']

        # View Centering
        padded = np.pad(map_data, self.pad, constant_values=self.pad_value)
...
class CroppedImagePCGRLWrapper(gym.Wrapper):
    def __init__(self, game, crop_size, **kwargs):
        ...
        env = Cropped(self.env, crop_size, self.env.get_border_tile(), 'map')
