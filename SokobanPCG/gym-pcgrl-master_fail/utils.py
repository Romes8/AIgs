...
def get_action(obs, env, model, action_type=True):
    ...
    if action_type == 0:
        ...
        action_prob = model.policy.predict_proba(obs)[0]
...
def make_vec_envs(env_name, representation, log_dir, n_cpu, **kwargs):
    ...
    '''
    if n_cpu > 1:
        env_lst = [make_env(env_name, representation, i, log_dir, **kwargs) for i in range(n_cpu)]
...
def max_exp_idx(exp_name):
    ...
    if len(log_files) == 0:
        ...
        log_ns = [re.search(r'_(\d+)', f).group(1) for f in log_files]
        n = max(log_ns)
    return int(n)
