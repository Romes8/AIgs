...
class NarrowRepresentation(Representation):
    ...
    def reset(self, width, height, prob):
        ...
        self._x = self._random.integers(0, width - 1)
        self._y = self._random.integers(0, height - 1)
    ...
    def update(self, action):
        # Check if action is a list or tuple and extract the intended action
        if isinstance(action, (list, tuple)):
            action = action[-1]  # Assume the last element represents the tile type
        ...
        if self._random_tile:
            self._x = self._random.integers(0, self._map.shape[1] - 1)
            self._y = self._random.integers(0, self._map.shape[0] - 1)
        else:
            self._x += 1
